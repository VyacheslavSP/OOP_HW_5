...
public class PhoneBook<T extends PhoneBookItem> implements ImportExport {
    ...
    public void appendRow(PhoneBookItem bookRow) {
        ArrayList<T> tmplist = this.list;
        tmplist.add((T) bookRow); // не понимаю почему тут надо кастовать к T. вроде как и T и bookRow есть
                                  // объекты с ограничением в PhoneBookItem
        setList(tmplist);
    }
    ...
    public void showBook() {
        ...
        while (iter.hasNext()) {
            ...
            System.out.println("номер телефона= " + elem.numberPhone.getInternationalСode()
                    ...
                    + Integer.toString(elem.numberPhone.PhoneBody)); /// Конструкция,конечно, получилась громоздкой,
                                                                     /// однако поля в телефоне специально храняться в
                                                                     /// разных форматах (например для организации
                                                                     /// поиска- выведи все номера с Москвы(код города
                                                                     /// 495))
                    ...
                    if (rows.get(0) == string_ID) {
                        if (rows.get(2).indexOf("+7") != -1) { // с текущими условиями проверка на стационарность через
                            ...
                            newStationaryPhone = new StationaryPhone(Integer.parseInt(rows.get(3)),
                                    Integer.parseInt(rows.get(4))); // копипаста текста. в будубщем планиеруется
                                                                    // отдельное поле мобилный/стационарный
                            mobile = !mobile;
                        }
                        // планируется поиск по ID человека. при этом сами человеки должны храниться в
                        // Мапе
